...
public class HookController : MonoBehaviour
{
    ...
    private bool IsBaitCompatibleWithFish(FishHookable fish)
    {
        ...
        // Get the fish size from its behavior
        FishSize fishSize = FishSize.Medium; // Default to medium
        
        // Determine fish size based on behavior size
        float size = fish.behavior.size;
        if (size <= 0.6f) fishSize = FishSize.Tiny;
        else if (size <= 0.8f) fishSize = FishSize.Small;
        else if (size <= 1.2f) fishSize = FishSize.Medium;
        else if (size <= 1.7f) fishSize = FishSize.Large;
        else fishSize = FishSize.Huge;
        
        // Check if this fish size is compatible with our bait
        return currentBait.IsAttractedToFish(fishSize);
    }
